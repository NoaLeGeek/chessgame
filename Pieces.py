...
class King(Piece):
    ...
    def get_available_moves(self, board, row, column, flipped: bool = False, **kwds):
        ...
        if self.column == (7 - flipped) // 2 and self.first_move:
            # O-O-O
            if isinstance(board[row][7 * (1 + flipped) // 2], Rook) and board[row][7 * (1 + flipped) // 2].first_move and all(board[row][i] == 0 for i in range((5 + 3 * flipped) // 2, 3 * (3 + flipped) // 2 + 1)):
                self.available_moves.append((row, (7 + 3 * flipped) // 2))
            # O-O
            if isinstance(board[row][7 * (1 - flipped) // 2], Rook) and board[row][7 * (1 - flipped) // 2].first_move and all(board[row][i] == 0 for i in range(3 - 2 * flipped, 2 * (2 - flipped) + 1)):
