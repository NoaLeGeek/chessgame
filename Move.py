...
class Move:
    ...
    def to_literal(self):
        ...
        if isinstance(self.piece, Pieces.King) and abs(self.from_[1] - self.to[1]) == 2:
            ...
            string += [(chr(flip_coord(self.game.flipped, self.from_[1]) + 97) if self.capture else ""), "N", "B", "R", "Q", "K"][Pieces.Piece.piece_to_index(self.piece)]
            # Add x if it's a capture
            string += ("x" if self.capture else "")
            # Add the destination's column
            string += chr(flip_coord(self.game.flipped, self.to[1]) + 97)
            # Add the destination's row
            string += str(flip_coord(-self.game.flipped, self.to[0]) + 1)
            # Add promotion
